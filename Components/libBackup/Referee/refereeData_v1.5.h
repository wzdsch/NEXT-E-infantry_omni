...
typedef __packed struct {
  ...
  uint8_t
      HP_LEFT : 1; //己方补给站左侧（面向补给站）补血点的占领状态，1 为已占领
  uint8_t
      HP_RIGHT : 1; //己方补给站右侧（面向补给站）补血点的占领状态，1 为已占领
...
typedef __packed struct {
  ...
  uint8_t
      mains_power_gimbal_output : 1; // gimbal 口输出：0 为无输出，1 为 24V 输出
...
typedef __packed struct {
  ...
  uint8_t
      dart_attack_target; //飞镖的打击目标：（默认为前哨站） 0:前哨站，1:基地
  uint16_t
      target_change_time; //切换打击目标时的比赛剩余时间，单位：s，无未切换动作默认为
  // 0。
  uint16_t
      latest_launch_cmd_time; //最后一次操作手确定发射指令时的比赛剩余时间，单位：s，初始值为
  // 0。
} dart_client_cmd_t;
