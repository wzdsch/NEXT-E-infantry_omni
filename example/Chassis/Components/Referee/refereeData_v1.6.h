...
typedef __packed struct {
  ...
  uint8_t HILL_CIRCLE_N2 : 2;  // 己方环形高地的占领状态，1为被己方占领，2为被对方占领
  uint8_t HILL_N3 : 2;  // 己方梯形高地的占领状态，1为被己方占领，2为被对方占领
  uint8_t HILL_N4 : 2;  // 己方梯形高地的占领状态，1为被己方占领，2为被对方占领

  uint8_t BASE_VIRTUAL_HP : 7;  // 己方基地虚拟护盾的剩余值百分比（四舍五入，保留整数）
  uint16_t The_last_time_hit : 9;  // 飞镖最后一次击中己方前哨站或基地的时间（0-420，开局默认为0）
  uint8_t
    The_outpost_base : 2;  // 飞镖最后一次击中己方前哨站或基地的具体目标，开局默认为0，1为击中前哨站，2为击中基地固定目标，3为击中基地随机目标
  uint8_t center_gain : 2;  // 中心增益点的占领情况，0 为未被占领，1 为被己方占领，2 为被对方占领，3
                            // 为被双方占领。
...
typedef __packed struct {
  ...
  uint8_t
    The_own_dart : 2;  // 最近一次己方飞镖击中的目标，开局默认为0，1为击中前哨站，2为击中基地固定目标，3为击中基地随机目标
  uint8_t Total_numbe : 3;  // 对方最近被击中的目标累计被击中计数，开局默认为0，至多为4
  uint8_t
    The_hit_the_target : 2;  // 飞镖此时选定的击打目标，开局默认或未选定/选定前哨站时为0，选中基地固定目标为1，选中基地随机目标为2
  uint16_t rest : 9;         // 保留
