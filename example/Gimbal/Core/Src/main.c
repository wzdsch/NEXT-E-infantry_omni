...
int main(void) {
  ...
  DJI_MotorInit(&motorYaw, 0x209, 0, pidBoth, 0);
  DJI_MotorPreProcessHandlerSet(&motorYaw,
                                &MIUecdZeroCrossing);  // 处理陀螺仪的数据，以及过零
  ...
  DJI_MotorInit(&motorPitch, 0x20a, 0, pidBoth, 6700);
  ...
  RemoteControlINIT(&rc_ctrl, &huart3);  // 遥控
  ...
  while (1) {
    ...
    gimbalRun(&gimbal1, yaw, pitch);
    GimbalChangeMode(&gimbal1, mode);
    ChassisControlData.speedx = rc_ctrl.rc.ch[2] * 10;
    ChassisControlData.speedy = rc_ctrl.rc.ch[3] * 10;
    ChassisControlData.speedz = 200;
    // shooterRun(&shooter1);
    tuneRun();
  }
